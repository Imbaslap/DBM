...
function Lurker:OnCombatStart(delay)
	...
	self:ScheduleSelf(375- delay, "SpoutWarning");
...
function Lurker:OnEvent(event, arg1)
	...
	elseif event == "Submerge" then
		...
		self:StartStatusBarTimer(45, "Emerge", "Interface\\AddOns\\DBM_API\\Textures\\CryptFiendUnBurrow");
		
	elseif event == "Emerge" then
		self.Submerged = false;
		self:Announce(DBM_LURKER_WARN_EMERGE, 3);
		self:StartStatusBarTimer(75, "Submerge", "Interface\\AddOns\\DBM_API\\Textures\\CryptFiendBurrow");
		self:EndStatusBarTimer("Emerge");
		self:ScheduleSelf(50, "SubmergeWarning", 25);
		self:ScheduleSelf(60, "SubmergeWarning", 15);
		self:ScheduleSelf(70, "SubmergeWarning", 5);
