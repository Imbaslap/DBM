...
function Maulgar:OnCombatStart(delay)
	...
	self:ScheduleSelf(25 - delay, "WhirlwindWarning");
	self:StartStatusBarTimer(45, "Felhunter", "Interface\\Icons\\Spell_Shadow_SummonFelHunter");
	self:ScheduleMethod(45, "SendSync", "Felhuntertwo");
	self:StartStatusBarTimer(11, " Next Prayer of Healing", "Interface\\Icons\\Spell_Holy_PrayerOfHealing02");
end
		...
		elseif arg1.spellId == 33054 then
			if bit.band(arg1.destFlags, COMBATLOG_OBJECT_TYPE_PLAYER) == 0 then
				if self.Options.WarnSpellShield then
					...
					self:StartStatusBarTimer(33, "Next Spell Shield", "Interface\\Icons\\Spell_Arcane_ArcaneResilience");
				end
			end
...
function Maulgar:OnSync(msg)
	if msg == "WhirlWind" then
		...
		self:StartStatusBarTimer(54, "Next Whirlwind", "Interface\\Icons\\Ability_Whirlwind");
		self:StartStatusBarTimer(15.2, "Whirlwind", "Interface\\Icons\\Ability_Whirlwind");
		self:ScheduleSelf(51, "WhirlwindWarning");
		
	elseif msg == "Felhuntertwo" then
		self:StartStatusBarTimer(50, "Felhunter", "Interface\\Icons\\Spell_Shadow_SummonFelHunter");
